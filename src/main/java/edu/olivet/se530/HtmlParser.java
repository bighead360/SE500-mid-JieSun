...
public class HtmlParser {
	...
	@Profile(desc = "解析一个给定的html document，返回其中的offer列表")
		...
		String productname = doc.select(selector).text();
	...
	Condition parseCondition(Element row) {
		String cond = this.getText(row, ".olpCondition");
		//old version
	...
	Seller parseSeller(Element row) {
        ...
        if(row.select(sellerNameSelector).text().length() != 0){
            ...
            String ratingText = this.getText(row, "div.olpSellerColumn > .a-spacing-small> a > b");
            if (ratingText != "") {
